...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["questionnaire_doc"]["tmp_name"], $target_file)) {
            $download_dir = "../account/downloads/";
            $destination_file = $download_dir . basename($_FILES["questionnaire_doc"]["name"]);
            ...
            if (copy($target_file, $destination_file)) {
                echo "The file has been uploaded and copied to the downloads directory.";
            } else {
                echo "Error copying the file to the downloads directory.";
                echo "Source: " . $target_file . "<br>";
            ...
            if ($stmt = $connection->prepare($sql)) {
                $stmt->bind_param("ssss", $title, $description, $destination_file, $comment);
                if ($stmt->execute()) {
...
<div class="main-container" id="container">
                        ...
                        <ul class="nav nav-tabs mb-3" id="pills-tab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="pills-add-new-tab" data-toggle="pill" href="#pills-add-new"
                                   role="tab" aria-controls="pills-add-new" aria-selected="false">Add New</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" id="pills-reports-tab" data-toggle="pill" href="#pills-reports"
                                   role="tab" aria-controls="pills-reports" aria-selected="false">Response</a>
                            </li>
                        </ul>
    ...
    <div class="form-group">
        <label for="recorded_date">Recorded Date<span class="required">*</span></label>
        <input type="date" name="recorded_date" id="recorded_date" class="form-control" required value="<?php echo date('Y-m-d'); ?>" disabled>
    </div>
